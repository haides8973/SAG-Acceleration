...
class GaussianDiffusion:
    ...
    :param rescale_timesteps:   if True, pass floating point timesteps into the model
                                so that they are always scaled like in the
                                original paper (0 to 1000).

    :param is_skippable:        a flag for enabling unet forward skipping, by directly passing 
                                feature map from the preivous forward.
    """
    ...
    def p_mean_variance(
        self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None, intermed_feat=None, first_t=False
    ):
        """
        ...
        :param intermed_feat: If not None, this feature map is directly feeded into 
                              UNet Upsampling block for the next model forward.
        :return: a dict with the following keys:
                 ...
                 - 'saved_feat': the UNet feature map from Downsampling block. 
                                If intermed_feat is not None, this must be None.
        """
        ...
        model_output, attn_map, saved_feat = model(x, self._scale_timesteps(t), intermed_feat, first_t, **model_kwargs)
        ...
        return {
            ...
            "attn_map": attn_map,
            "saved_feat": saved_feat if self.is_skippable else None
        }
    ...
    def p_sample(
        ...
        if attn_guidance and guide_start>t[0]:
            ...
            # MODIFIED : Also get the intermediate feature map to feed our unet for next model forward 
            saved_feat = out['saved_feat']
            ...
            sample = final_out["mean"] + nonzero_mask * torch.exp(0.5 * out["log_variance"]) * noise
            return {"sample": sample, "pred_xstart": out["pred_xstart"], "saved_feat": saved_feat}
    ...
    def p_sample_loop(
        ...
        :param guidance_kwargs: if not None, a dict of the parameters of self-attention guidance.
        :param skip_depth : if 0, it is not skippable.
        ...
        for i in indices:
            ...
            with torch.no_grad():
                out = self.p_sample(
                    ...
                    guidance_kwargs=guidance_kwargs,
                    intermed_feat=intermed_feat,
                    first_t=True if i == 1 else False 
                )
                yield out
                img = out["sample"]
                intermed_feat = out["saved_feat"]
    ...
    def ddim_sample(
        ...
        if attn_guidance and guide_start>t[0]:
            ...
            # MODIFIED : Also get the intermediate feature map to feed our unet for second model forward 
            saved_feat = out['saved_feat']
            ...
            noise = torch.randn_like(x)
            mean_pred = (
                final_out["pred_xstart"] * torch.sqrt(alpha_bar_prev)
